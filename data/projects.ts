...
// Internal-use figures only (e.g. aggregate dashboard charts) — private contract
// amounts are intentionally omitted from public-facing project listings/exports.
export const getProjectsFunding = (): ProjectFunding[] => {
  const filePath = path.join(process.cwd(), 'data', 'Projects.xlsx');
  const fileBuffer = fs.readFileSync(filePath);
  const wb = xlsx.read(fileBuffer, { type: 'buffer', cellDates: true });
  const ws = wb.Sheets[wb.SheetNames[0]];
  ...
  return data
    .filter(row => row['Tipo'] === 'Competitive' || row['Tipo'] === 'Private')
    .map((row) => {
      const tipo: 'Competitive' | 'Private' = row['Tipo'] === 'Competitive' ? 'Competitive' : 'Private';
      const rawStart = row['Duración (inicio)'];
      let startDate: Date | null = null;
      ...
      const rawMoney = row[' Asignación a la UPM '];
      const money = rawMoney != null && !isNaN(Number(rawMoney)) ? Number(rawMoney) : 0;
      const scope = row['Alcance'] ? String(row['Alcance']) : '';
      const isIP = typeof row['IP-YO'] === 'string' && row['IP-YO'].trim().toLowerCase() === 'yes';
      return { tipo, scope, isIP, startDate, money };
    });
};
