...
const parseRows = (data: any[], tipo: string, prefix: string): Publication[] => {
  return data.filter((row: any) => row['Tipo'] === tipo).map((row: any, index) => {
    ...
    return {
      ...
      journal: row['Revista'] ? String(row['Revista']) : '',
      type: tipo,
      location: row['Lugar'] ? String(row['Lugar']) : undefined,
      status,
      doi: row['Link'] ? String(row['Link']) : '',
      jcr: row['JCR'] ? String(row['JCR']) : '',
