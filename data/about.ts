...
export const getCertificates = (): AboutEntry[] =>
  loadAbout().filter(e => {
    const tipo = (e.tipo as string).toLowerCase();
    return tipo.startsWith('lang') || tipo.startsWith('sexenio');
  });

export const getInstitutionalRoles = (): AboutEntry[] =>
  loadAbout().filter(e => (e.tipo as string).toLowerCase().startsWith('institutional'));

export const getSexenios = (): AboutEntry[] =>
  loadAbout().filter(e => (e.tipo as string).toLowerCase().startsWith('sexenio'));
