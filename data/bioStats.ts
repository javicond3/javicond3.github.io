...
export function getBioStats(): BioStats {
  ...
  const supervisionStudents = tutorFiltered.filter(r => r['Tipo'] === 'Supervisor').length;
  ...
  // --- About.xlsx ---
  const about = readSheet('About.xlsx');
  const sexenioRow = about.find(r => String(r['Tipo'] ?? '').toLowerCase().startsWith('sexenio'));
  const sexenios = String(sexenioRow?.['Título'] ?? '');
