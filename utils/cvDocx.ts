...

  {
    ...
    if (hasAny) {
      ...
      if (hasSupervision) {
        ...
        const thesisItem = (thesis: { title: string; author: string; degree?: string; year?: number | string }) => item([
        ...
        if (supIdx.size > 0) {
          docChildren.push(h3("Scholarship Supervision", supIdx.size, allSupervisions.length));
